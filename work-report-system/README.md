# 程優教育科技｜技術團隊工作回報系統 — 部署包

這是一個**靜態網站**，部署後會得到兩個公開網址：一個給技術團隊每天回報，一個給 CEO 看儀表板。

## 📦 內容
| 檔案 | 說明 |
|---|---|
| `report.html` | **團隊回報頁**。手機優先，姓名+PIN 登入後逐筆填寫今日任務，送出即寫入試算表。 |
| `dashboard.html` | **CEO 儀表板**。3D 立體「誠信×產能」總覽（淡色漸層科技風）＋ 個人熱力圖／雷達圖／趨勢圖／任務明細。 |
...
| `建置與使用說明.md` | 完整部署與操作說明（含 CORS 注意事項、GitHub 模組啟用方式）。 |
...
- `https://xxxx.netlify.app/report.html` → 發給技術團隊
...
`API_URL` 留空時，兩個頁面會自動改讀同資料夾的 `demo-data.json`，用假資料把 3D 場景、象限分佈、熱力圖、雷達圖都跑起來，畫面會標示「🧪 示範資料」。這讓你在還沒建立 Apps Script 之前，就能直接雙擊打開 `dashboard.html` 看到完整視覺效果。
